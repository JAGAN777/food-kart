...
const Logs = props => {
    const {t, title, logs, Sub, offset,setOffset, isRefetching} = props
    const deliveryLogs = () => {
            ...
            {logs?.data?.map((log,index) => {
                return <CustomStackFullWidth direction='row' key={log?.id} alignItems='center'
                    ...
                    <Stack>
                        ...
                        <Typography fontSize='12px'>{getDateFormatAnotherWay(Sub?.start_at)}</Typography>
    ...
    return (
        <Paper sx={  {width: {xs: '100%', sm: '350px'}, padding: '1rem', minHeight: '400px'}}>
