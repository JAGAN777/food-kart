...
const BottomActions = props => {
    ...
    const handleSuccessCancelClick = (value)=>{
        if(Array.isArray(value)){
            ...
            const handleSuccess = ()=>{
                toast.success(t('This Subscription order skiped successfully.'))
                refetchAll?.()
            }
            if(value?.length!==0){
    ...
    return (<>    
        {/* <CustomPaperBigCard sx={{mt:'1rem'}}> */}
           ...
           <ModalView title='Are you sure to skip subscription?'  minDate={minDate} maxDate={maxDate} t={t} handleCancel={()=>setOpenPause(false)} handleSuccess={handleSuccessCancelClick}/>
