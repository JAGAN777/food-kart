...
export const getTotalPlanPrice = (items) => {
    let totalPrice = 0
    if (items?.length > 0) {
        items.map((item) => {
            totalPrice += item?.plans?.price
        })
        return totalPrice
    }
    return totalPrice
...
export const getTaxableTotalPrice2 = (items, couponDiscount, restaurantData) => {
    const isTaxIncluded =
        store?.getState?.()?.globalSettings?.global?.tax_included === 1
    let tax = restaurantData?.data?.tax
    let total =
...
export const handleProductValueWithOutDiscount = (product) => {
    ...
    if (product.variations?.length > 0) {
        productPrice += handleVariationValuesSum([])
...
export const getDeliveryFees = (
    ...
    if (restaurantData?.data?.free_delivery || orderType === 'take_away') {
        ...
        const result =  cartList.reduce((total, item) => total + parseFloat(item.plans?.delivery_charge), 0);
         return result ?? 0
    }
}
...
export const getCalculatedTotal2 = (
    cartList,
    couponDiscount,
    restaurantData,
    global,
...
export const getIndexFromArray = (arrayOfObjects, object) => {
    return arrayOfObjects.findIndex(
        (item) =>
            _.isEqual(item, object) &&
            item.id === object.id
    )
}
